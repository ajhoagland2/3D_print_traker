...
const MIN_AVAILABLE_SPOOL_GRAMS = 1;

export const DEFAULT_INVENTORY = [];

const LEGACY_DEFAULT_INVENTORY_IDS = new Set([
...
export function createInventoryItem(formData) {
  ...
  const isAvailable = isSpoolAvailable({ remainingWeight });
  const status = printerId && isAvailable ? String(formData.get("status") || "ready") : "reserve";
  ...
  return {
    ...
    status,
    printerId: isAvailable ? printerId : "",
...
export function createInventorySpoolFromProduct(product, overrides = {}) {
  ...
  return {
    ...
    status: isAvailable && overrides.printerId ? overrides.status || "ready" : "reserve",
    date_added: new Date().toISOString(),
    notes: String(overrides.notes || "").trim(),
    printerId: isAvailable ? overrides.printerId || "" : "",
...
export function normalizeInventory(inventory) {
  const incoming = Array.isArray(inventory) ? inventory : DEFAULT_INVENTORY;
  return incoming.filter((item) => !LEGACY_DEFAULT_INVENTORY_IDS.has(item.id)).map((item) => {
    ...
    const availableForPrinter = remainingWeight > MIN_AVAILABLE_SPOOL_GRAMS;
    const printerId = availableForPrinter ? item.printerId || "" : "";
    ...
    return {
      ...
      location,
      status: printerId ? item.status || "ready" : item.status || "reserve",
...
export function autoAssignReadySpools(jobs, inventory, printers) {
  ...
  if (!assignedInventory.length) return assignedInventory;
...
function reserveCandidates(inventory, demand) {
  ...
  return inventory
    .filter((item) => item.status === "reserve" && item.material === demand.material && isSpoolAvailable(item))
    .sort((a, b) => {
...
function matchingSpools(inventory, demand) {
  ...
  const reserve = inventory.filter(
    (item) => item.status === "reserve" && item.material === demand.material && isSpoolAvailable(item)
  );
...
function isSpoolAvailable(item) {
  return (Number(item.remainingWeight) || 0) > MIN_AVAILABLE_SPOOL_GRAMS;
}
