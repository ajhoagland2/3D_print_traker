...
import { normalizeProductionStatus } from "./productionAnalytics.js";
...
export function normalizeState(state) {
  ...
  const jobs = state.jobs.map((job) => ({
    ...
    status: normalizeProductionStatus(job.status, scheduledIds.has(job.id) ? "scheduled" : "queued"),
    customerPriority: job.customerPriority || job.priority || "normal",
  }));
